...
export class YeomanUI {
  ...
  private onGeneratorSuccess(
    ...
    const ignoreGen: boolean =
      this.generatorsToIgnoreArray.includes(generatorName);
    ...
    const message =
      this.uiOptions.messages.artifact_with_name_generated(generatorName);
  ...
  private onGenInstall(gen: any) {
    ...
    if (showProgress) {
      // NEW BEHAVIOR: Generator opts in - use enhanced progress notifications

      // Listen to writing phase
      gen.on("method:writing", () => {
        const projectName = getProjectName();
        this.youiEvents.doGeneratorProgress(
          projectName,
          "writing",
          showProgress
        );
      ...
      gen.on("method:end", () => {
        const projectName = getProjectName();
        this.youiEvents.doGeneratorProgress(projectName, "end", showProgress);
      });
    } else {
      // OLD BEHAVIOR: Generator doesn't opt in - use classic "Installing dependencies..." toast
      gen.on("method:install", () => {
  ...
  private async getGeneratorChoice(
    genData: GeneratorData,
    filter: GeneratorFilter,
    hiddenGeneratorsArray: string[]
  ) {
  ...
  private getGeneratorDestinationPath(destinationRoot: string): string {
    return isUriFlow(destinationRoot)
      ? destinationRoot
